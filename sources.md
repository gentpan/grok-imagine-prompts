# 数据来源

本项目整理自以下开源项目和社区贡献。感谢原作者的辛勤工作！

---

## 1. that-cod/awesome-grok-imagine-prompts

**GitHub**: https://github.com/that-cod/awesome-grok-imagine-prompts

**许可证**: MIT License

**描述**: 一个精心策划的 Grok Imagine 提示词集合，包含多个分类的提示词模板和示例。

**本项目使用情况**:
- ✅ 已收录: 183 个提示词
- ❌ 已排除: `nsfw.md` 中的所有内容
- ❌ 已排除: `scraper-log.md`（日志文件）

**收录分类**:
- 抽象与超现实 (Abstract & Surreal)
- 动作视频 (Action Videos)
- 动漫与赛博朋克 (Anime & Cyberpunk)
- 电影感 (Cinematic)
- 编辑与转换 (Editing & Transformation)
- 奇幻 (Fantasy)
- 杂项 (Misc)
- 自然 (Nature)
- 写实肖像 (Photorealistic Portraits)
- 科幻 (Sci-Fi)
- 风格与氛围 (Styles & Moods)
- 视频动画 (Video Animation)

---

## 2. seaimagine/awesome-grok-imagine-1-5-prompts

**GitHub**: https://github.com/seaimagine/awesome-grok-imagine-1-5-prompts

**许可证**: MIT License

**描述**: 40 个原创的、生产就绪的 Grok Imagine 1.5 视频提示词，覆盖电影叙事、UGC 广告、动漫动作、旅行、时尚、美食、科幻、喜剧、体育、音乐和房地产等场景。

**特点**:
- 🎬 专注于 Grok Imagine Video 1.5
- 📝 提供详细的提示词公式和模板
- 🌐 多语言支持（英语、中文、日语、西班牙语、韩语）
- ✨ 每个提示词包含推荐模式、时长、宽高比和可定制变量

**分类**:
- Cinematic storytelling (电影叙事)
- UGC & product ads (UGC 与产品广告)
- Anime & stylized action (动漫与风格化动作)
- Travel & lifestyle (旅行与生活方式)
- Fashion, beauty & food (时尚、美妆与美食)
- Fantasy & science fiction (奇幻与科幻)
- Comedy, memes & social (喜剧、梗与社交内容)
- Sports, music & spaces (体育、音乐与空间)

**在线资源**: https://seaimagine.com/model/grok-imagine-1-5/

---

## 3. YouMind-OpenLab/awesome-grok-imagine-prompts

**GitHub**: https://github.com/YouMind-OpenLab/awesome-grok-imagine-prompts

**许可证**: CC BY 4.0 (Creative Commons Attribution 4.0 International)

**描述**: 一个大型的 Grok Imagine 视频生成提示词库，包含 2500+ 精选提示词，涵盖电影、动作、动漫、产品、梗图等多种风格。

**特点**:
- 📊 大规模提示词库（2500+ 提示词）
- 🎬 视频播放和预览功能
- 🔍 多字段模糊搜索
- 🤖 AI 驱动的提示词推荐
- 📱 完全响应式设计

**在线画廊**: https://youmind.com/grok-imagine-prompts

**本项目使用情况**:
- 本项目收录 README 中的精选提示词预览
- 如需访问完整的 2500+ 提示词库，请访问原始仓库或在线画廊

---

## 4. imagineVid/awesome-grok-imagine-video-prompts-and-skills

**GitHub**: https://github.com/imagineVid/awesome-grok-imagine-video-prompts-and-skills

**许可证**: MIT License

**更新日期**: 2026-08-24

**描述**: 带有真实结果片段的 Grok Imagine 视频提示词，包含相机路径、音频感知工作流程和多语言指南。

**特点**:
- 🎥 带有视频结果的实战提示词
- 🎬 相机运动和镜头设计
- 🔊 音频同步工作流
- 🌐 多语言支持（15种语言）

**本项目使用情况**:
- ✅ 已收录: 24 个视频提示词
- 包含多语言元数据（中英文优先）

---

## 5. thoxakihiko/grok-imagine-prompt-1.5-guide

**GitHub**: https://github.com/thoxakihiko/grok-imagine-prompt-1.5-guide

**许可证**: CC BY 4.0

**更新日期**: 2026-06

**描述**: Grok Imagine 1.5 的完整提示词参考指南（图像 + 视频），实战测试的提示词参考手册，涵盖文生图、图像编辑、文生视频、图生视频、视频编辑等全流程。

**特点**:
- 📖 完整的技术规格和参数说明
- 🎯 经过验证的提示词模式
- ⚠️ 反模式和避坑指南
- 📝 可复制粘贴的模板

**本项目使用情况**:
- ✅ 已收录: 示例提示词
- 作为技术参考资料

---

## 6. medicinalsheep/grokmusicvideoprompt

**GitHub**: https://github.com/medicinalsheep/grokmusicvideoprompt

**许可证**: MIT License

**更新日期**: 2026-08-18

**描述**: 专为一致性 Grok 音乐视频生成设计的主提示词模板。

**本项目使用情况**:
- 作为音乐视频提示词的参考

---

## 7. Roulandu/dance-i2v-prompt

**GitHub**: https://github.com/Roulandu/dance-i2v-prompt

**许可证**: MIT License

**更新日期**: 2026-08-14

**描述**: Grok Imagine 1.5 卡点跳舞图生视频提示词 / Dance I2V Prompt

**本项目使用情况**:
- 作为图生视频（舞蹈）提示词的参考

---

## 原 3. YouMind-OpenLab/awesome-grok-imagine-prompts

**GitHub**: https://github.com/YouMind-OpenLab/awesome-grok-imagine-prompts

**许可证**: CC BY 4.0 (Creative Commons Attribution 4.0 International)

**描述**: 一个大型的 Grok Imagine 视频生成提示词库，包含 2500+ 精选提示词，涵盖电影、动作、动漫、产品、梗图等多种风格。

**特点**:
- 📊 大规模提示词库（2500+ 提示词）
- 🎬 视频播放和预览功能
- 🔍 多字段模糊搜索
- 🤖 AI 驱动的提示词推荐
- 📱 完全响应式设计

**在线画廊**: https://youmind.com/grok-imagine-prompts

**本项目使用情况**:
- 本项目仅收录 README 中的精选提示词预览
- 如需访问完整的 2500+ 提示词库，请访问原始仓库或在线画廊

---

## 整理说明

本项目对以上来源进行了以下整理工作：

1. **内容筛选**
   - 移除所有 NSFW 内容
   - 排除非 Grok Imagine 相关内容
   - 去重和质量控制

2. **格式统一**
   - 统一数据结构（JSONL 格式）
   - 规范化分类标签
   - 标准化元数据字段

3. **双语支持**
   - 为所有提示词添加中文标题
   - 提供中文说明和使用指导
   - 保持英文原文的完整性

4. **来源追溯**
   - 保留每个提示词的原始来源链接
   - 标注原始许可证信息
   - 记录整理者信息

---

## 许可证兼容性

本项目使用 **MIT License**，与以下来源许可证兼容：

- ✅ MIT License (that-cod, seaimagine) - 完全兼容
- ✅ CC BY 4.0 (YouMind-OpenLab) - 兼容，已注明归属

---

## 致谢

感谢以下项目和作者的贡献：

- [@that-cod](https://github.com/that-cod) - awesome-grok-imagine-prompts
- [@seaimagine](https://github.com/seaimagine) - awesome-grok-imagine-1-5-prompts
- [@YouMind-OpenLab](https://github.com/YouMind-OpenLab) - awesome-grok-imagine-prompts

以及所有在社区中分享 Grok Imagine 经验和技巧的创作者们！

---

**整理**: Yep ([gentpan](https://github.com/gentpan))

**最后更新**: 2026-08-24
